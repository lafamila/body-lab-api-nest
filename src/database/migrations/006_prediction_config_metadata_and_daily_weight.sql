...
with metadata_seed(kind, key, metadata) as (
  values
    ('global', 'fasting_threshold_hours', '{"description":"Hours without meals before fasting weight-loss logic starts.","setupText":"Set the number of meal-free hours before fasting begins to affect the forecast.","inputHint":"Use hours. Example: 10 means fasting adjustment starts after 10 hours.","unit":"hours","requiredInSetup":true}'::jsonb),
    ('global', 'fasting_max_hours', '{"description":"Maximum fasting duration considered by the prediction clamp.","setupText":"Set the longest fasting window the forecast should count.","inputHint":"Use hours. Values beyond this are capped.","unit":"hours","requiredInSetup":true}'::jsonb),
    ...
    ('global', 'delta_min_kg', '{"description":"Lower bound for one-day predicted weight change.","setupText":"Set the minimum daily prediction delta.","inputHint":"Use kg. This prevents an extreme negative forecast.","unit":"kg","requiredInSetup":true}'::jsonb),
    ('global', 'delta_max_kg', '{"description":"Upper bound for one-day predicted weight change.","setupText":"Set the maximum daily prediction delta.","inputHint":"Use kg. This prevents an extreme positive forecast.","unit":"kg","requiredInSetup":true}'::jsonb),
